...
#write.csv(analysis_data, file=file.path(root, "..\\2026\\202603\\Sanabil\\analysis_data.csv"))
```
...
# 4. Calculate Odds Ratios (OR) and 95% Confidence Intervals
odds_ratios <- exp(cbind(OR = coef(logit_model), confint(logit_model)))
print(odds_ratios)


table(analysis_data_clean$COI, analysis_data_clean$is_GW_favored)



boxplot(Sample_Size ~ is_GW_favored, data = analysis_data_clean)
boxplot(QUIN_24 ~ is_GW_favored, data = analysis_data_clean)
```


### inlcuding sample size as covariate 
...
# Extract Odds Ratios and 95% Profile Likelihood Confidence Intervals
firth_OR <- exp(cbind(
  OR = coef(firth_model), 
  confint(firth_model)
))
