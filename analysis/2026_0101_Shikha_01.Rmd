...
data1=read_xlsx((file.path(root, "../2026/202601/Shikha/Results -2026-01-22 (1).xlsx")))
...
data2=data1 %>% mutate(patient_tooth_id=paste0(data1$Patient,":",data1$Site))
...
unique(data2$Procedure)



# Count the number of each Procedure
procedure_counts <- data2 %>%dplyr::count(`Procedure`)
...
```{r, message=F, warning=F}
data2$POSTDate=as.Date(data2$POSTDate)
data2$Birth=as.Date(data2$Birth)
data2$Age <- floor(as.numeric(difftime(Sys.Date(), data2$Birth, units = "days")) / 365.25)

activity$TreatmentDate=as.Date(activity$TreatmentDate)
data2$`Fail Date`=as.Date(data2$`Fail Date`)
data2$`RCT TreatmentDate`=as.Date(data2$`RCT TreatmentDate`)
...
data3 <- data2 %>%
  ...
  ) %>% select(`RCT TreatmentDate`, POSTDate, RCT_to_treatment_days, patient_tooth_id, `Fail Date`, `Fail Code`, Producer, `AgeAt Treatment`, Sex,  )

data3$POSTDate=as.Date(data3$POSTDate)
...
data=data3 %>% filter(RCT_to_treatment_days>=0)
...
p=ggsurvplot(
  km_fit,
  data = endo2,  # ✅ This solves the error
  conf.int = T,
 ...
 censor.shape = NA,       # <-- hide the vertical censoring ticks
  ...
  risk.table.fontsize = 2.5,   # 👈 THIS controls the numbers
...
endo2 =endo2 %>% filter(Sex==1 | Sex==2)
...
# 1. Ensure Sex is a Factor (so R knows to compare Male vs Female)
# and ensure survival_days are valid (must be > 0)
endo3 <- endo3 %>%
  mutate(
    Sex = as.factor(Sex)
  )

# 2. Create the Survival Object
# 'event' (1=Fail, 0=Censored)
surv_obj <- Surv(time = endo3$survival_days, event = endo3$event)

# 3. Fit the Cox Proportional Hazards Model
# We include Age, Sex, and your previous predictor
cox_model <- coxph(surv_obj ~ `AgeAt Treatment` + Sex + RCT_to_treatment_days+Group, 
                   data = endo3)
...
1. Age At Treatment ($p = 0.0343$) — Significant. Hazard Ratio (HR): $0.9955$

Interpretation:  For every 1-year increase in a patient's age, the risk (hazard) of treatment failure decreases by 0.45% ($1 - 0.9955 = 0.0045$).

2. Sex2 ($p = 0.5501$) — Not Significant. Hazard Ratio (HR): $0.9534$

Interpretation:  There is no statistically significant difference in tooth failure risk between Sex 1 and Sex 2 in this model.

3. RCT to Treatment Days ($p < 2e-16$) — Highly Significant. 
Hazard Ratio (HR): $1.0004$

Interpretation: Because the HR is greater than 1, this is a risk factor. For every 1-day increase in the time between the RCT and the final treatment, the risk of failure increases by 0.04%.

4. GroupS ($p=0.0333$) —  Significant. Hazard Ratio (HR): $0.8355$

Interpretation: compared to GroupG, the risk of GroupS is decreased by $1-0.8355=16.45\%$. 
