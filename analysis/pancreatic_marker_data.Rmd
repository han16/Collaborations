...
pancreatic_data_coded_time_no_missing_tidy=as_tibble(traj[,,1])
colnames(pancreatic_data_coded_time_no_missing_tidy)=c("0", "2", "10")
rownames(pancreatic_data_coded_time_no_missing_tidy)=unique(pancreatic_data_coded_time_no_missing$study_id_num)

cluster.result=kmeans(pancreatic_data_coded_time_no_missing_tidy, 2)
center=tibble(traj=as.vector(t(cluster.result$centers)), time=rep(c("0", "2", "10"),2), group=rep(c("1", "2"),each=3))
ggplot(center, aes(x=time, y=traj, group=group))+
  ylab("ca199_long")+xlab("coded time")+
...
num.cluster=3
cluster.result=kmeans(pancreatic_data_coded_time_no_missing_tidy, num.cluster)
center=tibble(traj=as.vector(t(cluster.result$centers)), time=rep(c("0", "2", "10"),num.cluster), group=as.character(rep(seq(1:num.cluster),each=3)))
ggplot(center, aes(x=time, y=traj, group=group))+
  ylab("ca199_long")+xlab("coded time")+
  geom_line(aes(color=group))+
  geom_point(aes(color=group))
