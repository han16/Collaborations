...
data_23_clean$average=rowMeans(data_23_clean, na.rm = T)
data_24_clean=as.data.frame(data_clean(data_24))
data_24_clean$average=rowMeans(data_24_clean, na.rm = T)
...
* the last row is average across all previous rows 
...
```{r, message=F, warning=F}
# Compute Mean and SD, then round the results to 4 digits
summary_stats <- combined_data %>%
  group_by(year) %>%
  summarise(across(starts_with("probe"), 
...
summary_stats <- combined_data %>%
  group_by(year) %>%
  summarise(across(starts_with("Explorer"), 
                   list(mean = ~round(mean(.x, na.rm = TRUE), 4), 
                        sd = ~round(sd(.x, na.rm = TRUE), 4)), 
                   .names = "{.col}_{.fn}"))
...
summary_stats <- combined_data %>%
  group_by(year) %>%
  summarise(across(starts_with("Sickler"), 
                   list(mean = ~round(mean(.x, na.rm = TRUE), 4), 
                        sd = ~round(sd(.x, na.rm = TRUE), 4)), 
                   .names = "{.col}_{.fn}"))
...
summary_stats <- combined_data %>%
  group_by(year) %>%
  summarise(across(starts_with("Universal"), 
                   list(mean = ~round(mean(.x, na.rm = TRUE), 4), 
                        sd = ~round(sd(.x, na.rm = TRUE), 4)), 
                   .names = "{.col}_{.fn}"))
...
summary_stats <- combined_data %>%
  group_by(year) %>%
  summarise(across(starts_with("Area_Specific"), 
                   list(mean = ~round(mean(.x, na.rm = TRUE), 4), 
                        sd = ~round(sd(.x, na.rm = TRUE), 4)), 
                   .names = "{.col}_{.fn}"))
...
```{r, message=F, warning=F}
hand_test=read.csv((file.path(root, "..\\2025\\202503\\Zainab\\hand test.csv")))
summary_statistics(hand_test[,4:53])%>%
datatable(extensions = 'Buttons',
          caption = "",
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
