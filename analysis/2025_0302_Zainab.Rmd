...
# -------------------------------
df_bin <- df_long %>%
  mutate(binary = ifelse(response %in% c("Agree", "Strongly agree"), "Agree+", "Other")) %>%
  pivot_wider(names_from = time, values_from = binary)


mcnemar_results <- df_bin %>%
  group_by(question) %>%
  do({
    tbl <- table(.$pre, .$post)
...
```{r, message=F, warning=F}
# -------------------------------
# 4b. Stuart–Maxwell test (full Likert)
# -------------------------------
# rstatix provides `mcnemar_test`, but not Stuart–Maxwell.
# We can use DescTools::StuartMaxwellTest for multinomial paired data.
library(DescTools)

stuart_results <- df_long %>%
  pivot_wider(names_from = time, values_from = response) %>%
  group_by(question) %>%
  summarise(
    test = list(StuartMaxwellTest(table(pre, post))),
    .groups = "drop"
  )

print(stuart_results)
