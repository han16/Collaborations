...
project_progress=data.frame(project=seq(1,11), progress=c(100, 100, 100, 90, 10, 50, 100, 60, 60, 90, 70))
...
bp.x <- ggplot(data = project_progress, aes(y = factor(project), x = progress)) + 
    geom_bar(stat = "identity", aes(fill = project)) + coord_flip()+theme_gray() +
  #geom_text(aes(label=progress), position=position_dodge(width=0.9), vjust=-0.25)+  # add numbers over bars+
    labs(x = "project")+
   labs(y="% (estimated)")+
  scale_fill_distiller(name = "progress", palette = "Reds", direction = 1, na.value = "transparent") +
