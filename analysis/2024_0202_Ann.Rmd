...
colnames(data$intervention)[3:19]
```


```{r, message=F, warning=F}
num_indi1=nrow(data$intervention)
num_indi2=nrow(data$control)
variable="I understand and can describe what common musculoskeletal disorders (MSD) are:"
...
data.frame(variable=paste(common_variable, ":intervention_vs_control"), pvalue=pvalue)%>%
datatable(extensions = 'Buttons',
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1),
...
```{r, message=F, warning=F}
variable="I understand the impact MSD's can have on productivity as a future dental professional"
...
```{r, message=F, warning=F}
variable="I understand the importance of proper body mechanics"
