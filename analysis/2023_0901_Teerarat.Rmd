...
group_corr=data2$`Data CORR`
num_indi=24
Data=group_corr[1:num_indi,]
```


```{r, echo=F, message=F, warning=F, eval=F}
events=Data %>% dplyr::count(Gender) %>% mutate(percentage=n/sum(n))
if (nrow(events)==2)
{
 events%>%
datatable(extensions = 'Buttons',
          caption = "CORR",
            options = list(dom = 'Blfrtip',
                           ...
                           lengthMenu = list(c(10,25,50,-1),
                                             c(10,25,50,"All")))) 
pvalue_adjust(binom.test(events$n)$p.value)
}
```
...
```{r, echo=F, message=F, warning=F, eval=F}
Data %>% dplyr::count(Smoking) %>% mutate(percentage=round(n/sum(n),4))
events=Data %>% dplyr::count(Smoking) %>% select(n) %>% pull()
#pvalue_adjust(binom.test(events)$p.value)
```


```{r, echo=F, message=F, warning=F, eval=F}
Implant_site=Data$Implant_site[1:num_indi]
Pre_implant_Procedure=Data$Pre_implant_Procedure[1:num_indi]
I_placement=Data$I_placement[1:num_indi]
Comp_placement=Data$Comp_placement[1:num_indi]
HA=Data$HA[1:num_indi]
I_Brand=Data$I_Brand[1:num_indi]
I_Surface=Data$I_Surface[1:num_indi]
I_Shape=Data$I_Shape[1:num_indi]
I_Techique=Data$I_Technique[1:num_indi]
I_Location=Data$I_Location[1:num_indi]
Month_post_regen=as.numeric(Data$Month_post_regen)[1:num_indi]
RTR_post_regen_IP=as.numeric(Data$RTR_post_regen_IP)[1:num_indi]
RTR_post_regen_CRP=as.numeric(Data$RTR_post_regen_CRP)[1:num_indi]
Time_comp_post_regen=as.numeric(Data$Time_comp_post_regen)[1:num_indi]
...
data_interest_corr=data.frame(Gender=Gender, Smoking=Smoking,Implant_site=Implant_site, Pre_implant_Procedure=Pre_implant_Procedure, I_placement=I_placement, Comp_placement=Comp_placement, HA=HA, I_Brand=I_Brand, I_Surface=I_Surface, I_Shape=I_Shape,I_Techique=I_Techique, I_Location=I_Location)
