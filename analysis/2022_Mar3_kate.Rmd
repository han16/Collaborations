...
#### test group by time interaction 
...
summary(model)
 ...
 anova(model)
```
* use normalized cortisol values and no overall group by time interaction 


#### test group difference at every time point
...
num_participants[2]=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="F" & BMI.kg.m2 <35) %>%nrow 
average_sAHI[2]=mean(data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="F" & BMI.kg.m2 <35) %>% select(sAHI) %>% pull, na.rm=T)
...
* intermediate: `STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="F" & BMI.kg.m2 <35`
