...
data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=16 & Epworth.Score<=24 & sAHI>=5) %>% select(Patient.ID.Number, Epworth.Score, sAHI)
...
average_sAHI[2]=mean(data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4  & sAHI>=5) %>% filter(Remarks.A..Gender=="F" & BMI.kg.m2<35 |Remarks.A..Gender=="M" & STOP<2 ) %>% select(sAHI) %>% pull, na.rm=T)
...
average_sAHI[3]=mean(data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=5 & STOP.BANG.total<=8 & sAHI>=5) %>% select(sAHI) %>% pull, na.rm=T)
...
average_sAHI[4]=mean(data%>%filter(Patient.ID.Number>=40 & STOP==2 &  STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="M" & sAHI>=5 ) %>% select(sAHI) %>% pull, na.rm=T)
...
average_sAHI[5]=mean(data%>%filter(Patient.ID.Number>=40 & STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & BMI.kg.m2 >35 & sAHI>=5) %>% select(sAHI) %>% pull, na.rm=T)



knitr::kable(data.frame(Number_of_yes_response=c("0-2", "3-4", "5-8", "2 STOP+MALE GENDER", "2 STOP+bmi>35"), STOP_BANG_OSA_risk=c("low", "intermediate", "high", "high", "high"), num_participants=num_participants, Average_sAHI=round(average_sAHI,2)), "pipe")
