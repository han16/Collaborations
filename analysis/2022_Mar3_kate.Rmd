...
* group 1: `Average.PBI.Score.8.week <2`; group 2: `Average.PBI.Score.8.week >=2`
...
there are 17 individuals with normal cortisols across 4 timepoints. 


```{r, echo=T, warning=F, message=F}
prop.test(17,102, p=0.15)
```

* test the the rate of normal cortisol is 15%, the population rate or not. 

* p value of 0.74 indicates no difference 
...
* group 1: `Average.PBI.Score.8.week <2` and group 2: `Average.PBI.Score.8.week>=2`
* p value of 0.4678 for treatment X time interaction indicates lack of interaction. 


## three groups by SAI 

* three groups: group 1: `SAI<5`; group 2: `SAI>=5 & SAI<=14`; group 3: `SAI>14`
...
library(foreign)
############ define groups 
SAI_group1=data %>% filter(SAI<5) %>% select(Patient.ID.Number) %>% pull
SAI_group2=data %>% filter(SAI>=5 & SAI<=14) %>% select(Patient.ID.Number) %>% pull
SAI_group3=data %>% filter(SAI>14) %>% select(Patient.ID.Number) %>% pull
############ prepare the data 
Patient.ID.Number=data$Patient.ID.Number
trt=Patient.ID.Number
trt[trt%in%SAI_group1]="group1"
trt[trt%in%SAI_group2]="group2"
trt[trt%in%SAI_group3]="group3"
trt=factor(trt)

data_Cortisol_update1=data.frame(id=Patient.ID.Number, treatment=trt, y0=data_Cortisol$Cortisol.1, y1=data_Cortisol$Cortisol.2, y2=data_Cortisol$Cortisol.3, y3=data_Cortisol$Cortisol.4)

######## convert into long format 
data_Cortisol_update_long <- reshape(data_Cortisol_update1, idvar="id", varying=c("y0","y1","y2","y3"), v.names="y", timevar="time", time=1:4, direction="long")
attach(data_Cortisol_update_long) # when attaching variables, DO not use same variable names multiple times 

######### plot mean curve 
interaction.plot(time, treatment, y, type="b", pch=c(1,2,3), xlab="Time", ylab="Raw cortisol", main="", col=c("black", "blue", "red"))
...
#summary(model)
 ...
 anova(model)
```
*  no overall group by time interaction due to p value of 0.0903
* use cortisol raw values 


```{r, echo=F, warning=F, message=F}
data_Cortisol_update2=data.frame(id=Patient.ID.Number, treatment=trt, y0=data_Cortisol$Cortisol.1_normalized, y1=data_Cortisol$Cortisol.2_normalized, y2=data_Cortisol$Cortisol.3_normalized, y3=data_Cortisol$Cortisol.4_normalized)

######## convert into long format 
data_Cortisol_update_long <- reshape(data_Cortisol_update2, idvar="id", varying=c("y0","y1","y2","y3"), v.names="y", timevar="time", time=1:4, direction="long")
attach(data_Cortisol_update_long) # when attaching variables, DO not use same variable names multiple times 

######### plot mean curve 
interaction.plot(time, treatment, y, type="b", pch=c(1,2,3), xlab="Time", ylab="Normalized cortisol", main="", col=c("black", "blue", "red"))
...
*  no overall group by time interaction due to p value of 0.26
* use cortisol normalized values 

### test cortisol group difference at every time point

```{r, echo=F, message=F, warning=F}
annova_pvalue=numeric()
for (i in 1:4)
{
  Cortisol_group1=data_Cortisol_update1[which(data_Cortisol_update1$treatment=="group1"),(i+2)]
  Cortisol_group2=data_Cortisol_update1[which(data_Cortisol_update1$treatment=="group2"),(i+2)]
  Cortisol_group3=data_Cortisol_update1[which(data_Cortisol_update1$treatment=="group3"),(i+2)]
  annova_data=data.frame(cortisol=c(Cortisol_group1, Cortisol_group2, Cortisol_group3), groups=c(rep("group1", length(Cortisol_group1)), rep("group2", length(Cortisol_group2)), rep("group3", length(Cortisol_group3))))
  LM=lm(cortisol~ groups, data=annova_data)
  annova_pvalue[i]=anova(LM)[1,5]
}
annova_pvalue
```
* use cortisol raw values 
...
```{r, echo=F, warning=F, message=F}
PBI_week8_group1=data%>%filter(Patient.ID.Number %in% SAI_group1) %>% select(Average.PBI.Score.8.week) %>% pull
PBI_week8_group2=data%>%filter(Patient.ID.Number %in% SAI_group2) %>% select(Average.PBI.Score.8.week) %>% pull
PBI_week8_group3=data%>%filter(Patient.ID.Number %in% SAI_group3) %>% select(Average.PBI.Score.8.week) %>% pull
annova_data=data.frame(PBI_week8=c(PBI_week8_group1, PBI_week8_group2, PBI_week8_group3), groups=c(rep("group1", length(PBI_week8_group1)), rep("group2", length(PBI_week8_group2)), rep("group3", length(PBI_week8_group3))))
  LM=lm(PBI_week8~ groups, data=annova_data)
  anova(LM)
mean(PBI_week8_group1)
mean(PBI_week8_group2)
mean(PBI_week8_group3)
```
* use average PBI 
...
```{r, echo=F, warning=F, message=F}
SQI_group1=data%>%filter(Patient.ID.Number %in% SAI_group1) %>% select(SQI) %>% pull
SQI_group2=data%>%filter(Patient.ID.Number %in% SAI_group2) %>% select(SQI) %>% pull
SQI_group3=data%>%filter(Patient.ID.Number %in% SAI_group3) %>% select(SQI) %>% pull
annova_data=data.frame(SQI=c(SQI_group1, SQI_group2, SQI_group3), groups=c(rep("group1", length(SQI_group1)), rep("group2", length(SQI_group2)), rep("group3", length(SQI_group3))))
  LM=lm(SQI~ groups, data=annova_data)
  anova(LM)
mean(SQI_group1)
mean(SQI_group2)
mean(SQI_group3)
```
* average SQI: group 1: 67.21; group 2: 51.39; group 3: 38.18

* p value of `3.463e-16` indicates strong evidence of group difference  

## two groups by sAHI

* two groups: group 1: `sAHI<5`; group 2: `sAHI>=5`
...
library(foreign)
############ define groups 
sAHI_group1=data %>% filter(sAHI<5) %>% select(Patient.ID.Number) %>% pull
sAHI_group2=data %>% filter(sAHI>=5) %>% select(Patient.ID.Number) %>% pull
############ prepare the data 
Patient.ID.Number=data$Patient.ID.Number
trt=Patient.ID.Number[32:102]
trt[trt%in%sAHI_group1]="group1"
trt[trt%in%sAHI_group2]="group2"
trt=factor(trt)
data_Cortisol_update3=data.frame(id=Patient.ID.Number[32:102], treatment=trt, y0=data_Cortisol$Cortisol.1[32:102], y1=data_Cortisol$Cortisol.2[32:102], y2=data_Cortisol$Cortisol.3[32:102], y3=data_Cortisol$Cortisol.4[32:102]) 
#%>% filter(Patient.ID.Number>=40) # remove rows with NA sAHI

#data_Cortisol_update=data_Cortisol_update %>% filter(Patient.ID.Number>=40) # remove rows with NA sAHI
######## convert into long format 
data_Cortisol_update_long <- reshape(data_Cortisol_update3, idvar="id", varying=c("y0","y1","y2","y3"), v.names="y", timevar="time", time=1:4, direction="long")
attach(data_Cortisol_update_long) # when attaching variables, DO not use same variable names multiple times 

######### plot mean curve 
interaction.plot(time, treatment, y, type="b", pch=c(19,21), xlab="Time", ylab="Raw cortisol", main="", col=c(2,4))
...
* only 71 individuals are included as others have `NA` sAHI. 
* use normalized cortisol values and no overall group by time interaction due to p value of 0.7137
* use cortisol raw values 
...
for (i in 1:4)
{
  Cortisol_group1=data_Cortisol_update3[which(data_Cortisol_update3$treatment=="group1"),(i+2)]
  Cortisol_group2=data_Cortisol_update3[which(data_Cortisol_update3$treatment=="group2"),(i+2)]
  annova_data=data.frame(cortisol=c(Cortisol_group1, Cortisol_group2), groups=c(rep("group1", length(Cortisol_group1)), rep("group2", length(Cortisol_group2))))
  LM=lm(cortisol~ groups, data=annova_data)
  annova_pvalue[i]=anova(LM)[1,5]
}
annova_pvalue
...
```{r, echo=F, warning=F, message=F}
PBI_week8_group1=data%>%filter(Patient.ID.Number %in% sAHI_group1) %>% select(Average.PBI.Score.8.week) %>% pull
PBI_week8_group2=data%>%filter(Patient.ID.Number %in% sAHI_group2) %>% select(Average.PBI.Score.8.week) %>% pull
annova_data=data.frame(PBI_week8=c(PBI_week8_group1, PBI_week8_group2), groups=c(rep("group1", length(PBI_week8_group1)), rep("group2", length(PBI_week8_group2))))
  LM=lm(PBI_week8~ groups, data=annova_data)
  anova(LM)
mean(PBI_week8_group1)
mean(PBI_week8_group2)  
```
* use average PBI 
* group averages are group 1: 2.29; group 2: 2.33
...
```{r, echo=F, warning=F, message=F}
SQI_group1=data%>%filter(Patient.ID.Number %in% sAHI_group1) %>% select(SQI) %>% pull
SQI_group2=data%>%filter(Patient.ID.Number %in% sAHI_group2) %>% select(SQI) %>% pull
annova_data=data.frame(SQI=c(SQI_group1, SQI_group2), groups=c(rep("group1", length(SQI_group1)), rep("group2", length(SQI_group2))))
  LM=lm(SQI~ groups, data=annova_data)
  anova(LM)
mean(SQI_group1)
mean(SQI_group2)
```
* average SQI: group 1: 54.82; group 2: 42.88

* p value of `0.0028` indicates strong evidence of group difference  
...
data1=data%>%filter(Patient.ID.Number>=40 &Epworth.Score>=0 & Epworth.Score<=5) %>% select(sAHI) %>% pull
average_sAHI[1]=mean(data1, na.rm=T)
SD[1]=sd(data1, na.rm=T)
CI_low[1]=t.test(data1)$conf.int[1]
CI_upper[1]=t.test(data1)$conf.int[2]

num_participants[2]=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=6 & Epworth.Score<=10) %>%nrow 
data2=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=6 & Epworth.Score<=10) %>% select(sAHI) %>% pull
average_sAHI[2]=mean(data2, na.rm=T)
SD[2]=sd(data2, na.rm=T)
CI_low[2]=t.test(data2)$conf.int[1]
CI_upper[2]=t.test(data2)$conf.int[2]

num_participants[3]=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=11 & Epworth.Score<=12) %>%nrow 
data3=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=11 & Epworth.Score<=12) %>% select(sAHI) %>% pull
average_sAHI[3]=mean(data3, na.rm=T)
SD[3]=sd(data3, na.rm=T)
CI_low[3]=t.test(data3)$conf.int[1]
CI_upper[3]=t.test(data3)$conf.int[2]

num_participants[4]=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=13 & Epworth.Score<=15) %>%nrow
data4=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=13 & Epworth.Score<=15) %>% select(sAHI) %>% pull
average_sAHI[4]=mean(data4, na.rm=T)
SD[4]=sd(data4, na.rm=T)
CI_low[4]=NA
CI_upper[4]=NA

num_participants[5]=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=16 & Epworth.Score<=24) %>%nrow
data5=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=16 & Epworth.Score<=24) %>% select(sAHI) %>% pull
average_sAHI[5]=mean(data5, na.rm=T)
SD[5]=sd(data5, na.rm=T)
CI_low[5]=t.test(data5)$conf.int[1]
CI_upper[5]=t.test(data5)$conf.int[2]

data.frame(Epworth_score=c("0-5", "6-10", "11-12", "13-15", "16-24"), sleepiness_severity=c("lower normal", "higher normal", "mild excessive", "moderate excessive", "severe"), num_participants=num_participants, Average_sAHI=round(average_sAHI,2), SD=round(SD,2), CI_low=round(CI_low,2), CI_upper=round(CI_upper, 2))%>% datatable(extensions = 'Buttons',
...
data1=data%>%filter(Patient.ID.Number>=40 &STOP.BANG.total>=0 & STOP.BANG.total<=2) %>% select(sAHI) %>% pull
average_sAHI[1]=mean(data1, na.rm=T)
SD[1]=sd(data1, na.rm=T)
CI_low[1]=t.test(data1)$conf.int[1]
CI_upper[1]=t.test(data1)$conf.int[2]
...
data2=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP!=2 &  STOP.BANG.total<=4  & BMI.kg.m2 <35) %>% select(sAHI) %>% pull
average_sAHI[2]=mean(data2, na.rm=T)
SD[2]=sd(data2, na.rm=T)
CI_low[2]=t.test(data2)$conf.int[1]
CI_upper[2]=t.test(data2)$conf.int[2]
...
data3=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=5 & STOP.BANG.total<=8) %>% select(sAHI) %>% pull
average_sAHI[3]=mean(data3, na.rm=T)
SD[3]=sd(data3, na.rm=T)
CI_low[3]=t.test(data3)$conf.int[1]
CI_upper[3]=t.test(data3)$conf.int[2]
...
data4=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & STOP==2 & Remarks.A..Gender=="M") %>% select(sAHI) %>% pull
average_sAHI[4]=mean(data4, na.rm=T)
SD[4]=sd(data4, na.rm=T)
CI_low[4]=t.test(data4)$conf.int[1]
CI_upper[4]=t.test(data4)$conf.int[2]

num_participants[5]=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & STOP==2 & BMI.kg.m2 >35) %>%nrow
data5=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & STOP==2 & BMI.kg.m2 >35) %>% select(sAHI) %>% pull
average_sAHI[5]=mean(data5, na.rm=T)
SD[5]=sd(data5, na.rm=T)
CI_low[5]=t.test(data5)$conf.int[1]
CI_upper[5]=t.test(data5)$conf.int[2]
...
data.frame(Number_of_yes_response=c("0-2", "3-4", "5-8", "2 STOP+MALE GENDER", "2 STOP+bmi>35"), STOP_BANG_OSA_risk=c("low", "intermediate", "high", "high", "high"), num_participants=num_participants, Average_sAHI=round(average_sAHI,2), SD=round(SD,2), CI_low=round(CI_low,2), CI_upper=round(CI_upper, 2))%>% datatable(extensions = 'Buttons',
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1),
                                             c(10,25,50,"All"))))
...
* low:  `STOP.BANG.total>=0 & STOP.BANG.total<=2`
* intermediate: `STOP.BANG.total>=3 & STOP.BANG.total<=4 & STOP!=2 & BMI.kg.m2 <35`
...
data1=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=0 & Epworth.Score<=5 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[1]=mean(data1, na.rm=T)
SD[1]=sd(data1, na.rm=T)
CI_low[1]=t.test(data1)$conf.int[1]
CI_upper[1]=t.test(data1)$conf.int[2]
...
data2=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=6 & Epworth.Score<=10 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[2]=mean(data2, na.rm=T)
SD[2]=sd(data2, na.rm=T)
CI_low[2]=t.test(data2)$conf.int[1]
CI_upper[2]=t.test(data2)$conf.int[2]
...
data3=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=11 & Epworth.Score<=12 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[3]=mean(data3, na.rm=T)
SD[3]=sd(data3, na.rm=T)
CI_low[3]=t.test(data3)$conf.int[1]
CI_upper[3]=t.test(data3)$conf.int[2]
...
data4=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=13 & Epworth.Score<=15 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[4]=mean(data4, na.rm=T)
SD[4]=NA
CI_low[4]=NA
CI_upper[4]=NA
...
data5=data%>%filter(Patient.ID.Number>=40 & Epworth.Score>=16 & Epworth.Score<=24 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[5]=mean(data5, na.rm=T)
SD[5]=sd(data5, na.rm=T)
CI_low[5]=t.test(data5)$conf.int[1]
CI_upper[5]=t.test(data5)$conf.int[2]

data.frame(Epworth_score=c("0-5", "6-10", "11-12", "13-15", "16-24"), sleepiness_severity=c("lower normal", "higher normal", "mild excessive", "moderate excessive", "severe"), num_participants_sAHI_larger_than5=num_participants, Average_sAHI=round(average_sAHI,2), SD=round(SD,2), CI_low=round(CI_low,2), CI_upper=round(CI_upper, 2))%>% datatable(extensions = 'Buttons',
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1),
                                             c(10,25,50,"All"))))
...
```
* ID>=40 `Patient.ID.Number>=40`
* boundary value are included


```{r, echo=F, message=F, warning=F}
Epworth1=data%>%filter(Patient.ID.Number>=40 & sAHI>=5) %>%select(Epworth.Score) %>%pull()
Epworth2=data%>%filter(Patient.ID.Number>=40 & sAHI<5) %>%select(Epworth.Score) %>%pull()
t.test(Epworth1, Epworth2)
```
* p value of 0.038< 5% indicates Epworth between group `sAHI>=5` and `sAHI<5` are statistically different. 
...
```{r, echo=F, warning=F, message=F}
data=as_tibble(read.csv("C:\\Shengtong\\Research\\AllCollaboration\\2022\\202203\\Kate\\June_2022_Final_Data.csv", header=T)) # this is newly corrected data
ID40_SB_sAHI5=as_tibble(read.csv("C:\\Shengtong\\Research\\AllCollaboration\\2022\\202203\\Kate\\ID40+SB sAHI5.csv", header=T))
```
...
CI_low=numeric()
CI_upper=numeric()
num_participants[1]=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=0 & STOP.BANG.total<=2 & sAHI>=5) %>%nrow 
data1=data%>%filter(Patient.ID.Number>=40 &STOP>=0 & STOP<=2 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[1]=mean(data1, na.rm=T)
SD[1]=sd(data1, na.rm=T)
CI_low[1]=t.test(data1)$conf.int[1]
CI_upper[1]=t.test(data1)$conf.int[2]

num_participants[2]=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4  & sAHI>=5) %>% filter(Remarks.A..Gender=="F" & BMI.kg.m2<35 |Remarks.A..Gender=="M" & STOP<2 ) %>%nrow 
data2=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=3 & STOP.BANG.total<=4  & sAHI>=5) %>% filter(Remarks.A..Gender=="F" & BMI.kg.m2<35 |Remarks.A..Gender=="M" & STOP<2 ) %>% select(sAHI) %>% pull
average_sAHI[2]=mean(data2, na.rm=T)
SD[2]=sd(data2, na.rm=T)
CI_low[2]=t.test(data2)$conf.int[1]
CI_upper[2]=t.test(data2)$conf.int[2]

data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=5 & STOP.BANG.total<=8 & sAHI>=5) %>% select(Patient.ID.Number, STOP.BANG.total, sAHI)
num_participants[3]=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=5 & STOP.BANG.total<=8 & sAHI>=5) %>%nrow 
data3=data%>%filter(Patient.ID.Number>=40 & STOP.BANG.total>=5 & STOP.BANG.total<=8 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[3]=mean(data3, na.rm=T)
SD[3]=sd(data3, na.rm=T)
CI_low[3]=t.test(data3)$conf.int[1]
CI_upper[3]=t.test(data3)$conf.int[2]

data%>%filter(Patient.ID.Number>=40 & STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="M" & sAHI>=5 )%>% select(Patient.ID.Number, STOP.BANG.total, Remarks.A..Gender, sAHI)
num_participants[4]=data%>%filter(Patient.ID.Number>=40 & STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="M" & sAHI>=5 ) %>%nrow 
data4=data%>%filter(Patient.ID.Number>=40 & STOP==2 &  STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="M" & sAHI>=5 ) %>% select(sAHI) %>% pull
average_sAHI[4]=mean(data4, na.rm=T)
SD[4]=sd(data4, na.rm=T)
CI_low[4]=t.test(data4)$conf.int[1]
CI_upper[4]=t.test(data4)$conf.int[2]

data%>%filter(Patient.ID.Number>=40 & STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 &  BMI.kg.m2 >35 & sAHI>=5)%>% select(Patient.ID.Number, STOP.BANG.total, BMI.kg.m2, sAHI)
num_participants[5]=data%>%filter(Patient.ID.Number>=40 & STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 &  BMI.kg.m2 >35 & sAHI>=5) %>%nrow 
data5=data%>%filter(Patient.ID.Number>=40 & STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & BMI.kg.m2 >35 & sAHI>=5) %>% select(sAHI) %>% pull
average_sAHI[5]=mean(data5, na.rm=T)
SD[5]=sd(data5, na.rm=T)
#CI_low[5]=t.test(data5)$conf.int[1]
#CI_upper[5]=t.test(data5)$conf.int[2]
...
data.frame(Number_of_yes_response=c("0-2", "3-4", "5-8", "2 STOP+MALE GENDER", "2 STOP+bmi>35"), STOP_BANG_OSA_risk=c("low", "intermediate", "high", "high", "high"), num_participants=num_participants, Average_sAHI=round(average_sAHI,2), SD_sAHI=round(SD,2))%>% datatable(extensions = 'Buttons',
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1),
                                             c(10,25,50,"All"))))
...
* boundary value are included
* `sAHI>=5`
* low, 0-2: `STOP.BANG.total>=0 & STOP.BANG.total<=2`
* intermediate, 3-4: `STOP.BANG.total>=3 & STOP.BANG.total<=4  & sAHI>=5` and `Remarks.A..Gender=="F" & BMI.kg.m2<35 |Remarks.A..Gender=="M" & STOP<2 `
* high, 5-8: `STOP.BANG.total>=5 & STOP.BANG.total<=8 & sAHI>=5`
* high, 2 STOP+male gender: `STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 & Remarks.A..Gender=="M" & sAHI>=5`
* high, 2 STOP+BMI>35: `STOP==2 & STOP.BANG.total>=3 & STOP.BANG.total<=4 &  BMI.kg.m2 >35 & sAHI>=5`
...
data1=data%>%filter(Patient.ID.Number>=40) %>% select(BMI.kg.m2) %>% pull
average_BMI[1]=mean(data1, na.rm=T)
SD[1]=sd(data1, na.rm=T)
CI_low[1]=t.test(data1)$conf.int[1]
CI_upper[1]=t.test(data1)$conf.int[2]

num_participants[2]=data%>%filter(Patient.ID.Number>=40 & sAHI<5) %>%nrow 
data2=data%>%filter(Patient.ID.Number>=40 & sAHI<5) %>% select(BMI.kg.m2) %>% pull
average_BMI[2]=mean(data2, na.rm=T)
SD[2]=sd(data2, na.rm=T)
CI_low[2]=t.test(data2)$conf.int[1]
CI_upper[2]=t.test(data2)$conf.int[2]

num_participants[3]=data%>%filter(Patient.ID.Number>=40 & sAHI>=5) %>%nrow 
data3=data%>%filter(Patient.ID.Number>=40 & sAHI>=5) %>% select(BMI.kg.m2) %>% pull
average_BMI[3]=mean(data3, na.rm=T)
SD[3]=sd(data3, na.rm=T)
CI_low[3]=t.test(data3)$conf.int[1]
CI_upper[3]=t.test(data3)$conf.int[2]

num_participants[4]=data%>%filter(Patient.ID.Number>=40 & sAHI>=15) %>%nrow 
data4=data%>%filter(Patient.ID.Number>=40 & sAHI>=15) %>% select(BMI.kg.m2) %>% pull
average_BMI[4]=mean(data4, na.rm=T)
SD[4]=sd(data4, na.rm=T)
CI_low[4]=t.test(data4)$conf.int[1]
CI_upper[4]=t.test(data4)$conf.int[2]

data.frame(Epworth_score=c(" all participants", "sAHI<5", "sAHI>=5", "sAHI>=15"), num_participants=num_participants, Average_BMI=round(average_BMI,2), SD=round(SD,2), CI_low=round(CI_low,2), CI_upper=round(CI_upper, 2))%>% datatable(extensions = 'Buttons',
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1),
                                             c(10,25,50,"All"))))
...
```{r, echo=T, warning=F, message=F}
data%>%filter(Patient.ID.Number>=40 ) %>%nrow # No. participants 
```

```{r, echo=T, warning=F, message=F}
data%>%filter(Patient.ID.Number>=40 & sAHI>=5 ) %>%nrow # No. participants with sAHI>=5
```

```{r, echo=T, warning=F, message=F}
data%>%filter(Patient.ID.Number>=40 & sAHI>=5 & Epworth.Score==1) %>% select(Patient.ID.Number, sAHI, Epworth.Score, Average.PBI.Score.8.week) # No. participants with sAHI>=5
```

```{r, echo=T, warning=F, message=F}
data%>%filter(Patient.ID.Number>=40 & sAHI>=5 & Average.PBI.Score.8.week >2) %>%nrow # No. participants with sAHI>=5 & high risk individual with PBI>2
```

* From Figure 5, there are 6 STOP-BANG "high risk". 
...
```{r, echo=F, message=F, warning=F}
BMI_group1=data %>% filter(sAHI<5) %>% select(BMI.kg.m2) %>% pull
BMI_group2=data %>% filter(sAHI>=5) %>% select(BMI.kg.m2) %>% pull
t.test(BMI_group1, BMI_group2)
```
* the BMI difference between `sAHI<5` and `sAHI>=5` is significant  with p value of 0.02
* two sample t test was used. 
* the mean of `sAHI<5` is 24.52 and `sAHI>=5` is 27.32. 




### PBI is more reliable than STOP-Bang Statistically 

```{r, echo=T, message=F, warning=F}
prop.test(x=c(21,6), n=c(33,33), p = NULL, alternative = "greater",
          correct = TRUE)
```

* p value <0.001 indicating the ratio of PBI is statistically higher than that of STOP-Bang. 
...
```{r, echo=T, message=F, warning=F}
prop.test(x=c(21,2), n=c(33,33), p = NULL, alternative = "greater",
          correct = TRUE)
```

* p value <0.001 indicating the ratio of PBI is statistically higher than that of ESS. 
* two sample proportion test was used. 
