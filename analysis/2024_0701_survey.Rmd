...
{
   figures[[i]]=ggplot(specialty_geo_data %>% filter(geo %in% region_code[i]), aes(x=spect, y=num, fill=spect)) +geom_bar(position = "dodge", stat="identity")+
  ylim(c(0, max(specialty_geo_data %>% filter(geo %in% region_code[i]) %>% select(num)%>% pull())+10))+
  facet_grid("geo")+
  ylab("n")+xlab("")+
  theme(legend.position="")+
  theme(axis.text.x = element_text(angle = 30, vjust = 0.5, size=10))+
...
ggarrange(figures[[1]], figures[[2]], figures[[3]], figures[[4]], figures[[5]], figures[[6]], figures[[7]], figures[[8]], figures[[9]], figures[[10]], figures[[11]], figures[[12]], figures[[13]], ncol=2)
