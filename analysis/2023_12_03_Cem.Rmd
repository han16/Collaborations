...
for (i in 1:nrow(data_ssc_survival))
  survival_time[i]=sum(as.numeric(data_ssc_survival[i,2:12])==1)*6 # survival time in months 
...
Kaplan_Meier = with(data_ssc_survival_complete, Surv(survival_time, status))
Kaplan_Meier_fit=survfit(Surv(survival_time, status) ~ 1, data=data_ssc_survival_complete)
summary(Kaplan_Meier_fit)
plot(Kaplan_Meier_fit)
...
survfit2(Surv(survival_time, status) ~ 1, data = data_ssc_survival_complete %>% filter(`SSC Placement Technique`==1)) %>% 
  ggsurvfit() +
  labs(
    x = "Months",
    y = "survival probability of TT"
  ) + 
  add_confidence_interval()+
  add_risktable()

survfit2(Surv(survival_time, status) ~ 1, data = data_ssc_survival_complete %>% filter(`SSC Placement Technique`==2)) %>% 
  ggsurvfit() +
  labs(
    x = "Months",
    y = "survival probability of HT"
  ) + 
  add_confidence_interval()+
  add_risktable()
```
