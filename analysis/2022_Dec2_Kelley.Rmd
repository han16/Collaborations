...
###########
ggplot(data %>% count(Extraction) %>%mutate(prop=n/nrow(data)), aes(x=Extraction, y=n, fill=as.character(Extraction))) +geom_bar(stat="identity")+
...
###########
ggplot(data %>% count(Sex) %>%mutate(prop=n/nrow(data)), aes(x=Sex, y=n, fill=as.character(Sex))) +geom_bar(stat="identity")+
  theme(legend.position="none")+
...
###########
ggplot(data %>% count(CustomCode1) %>%mutate(prop=n/nrow(data)), aes(x=CustomCode1, y=n, fill=as.character(CustomCode1))) +geom_bar(stat="identity")+
  theme(legend.position="none")+
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, size=10))+
  geom_text(aes(label=n), position=position_dodge(width=0.6), vjust=-0.25)+  # add numbers over bars
  ggtitle("")+
