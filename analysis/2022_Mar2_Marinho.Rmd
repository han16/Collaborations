...
for (i in 1:length(phase))
  ...
  {
    mean[k]=round(mean(data_phase %>% filter(time==phase[i] & Sex==gender[j]) %>% select(variable) %>% pull),4) 
    sd[k]=round(sd(data_phase %>% filter(time==phase[i] & Sex==gender[j]) %>% select(variable) %>% pull),4)
    k=k+1
  }
