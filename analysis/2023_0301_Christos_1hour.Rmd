...
sav=forest(m.gen, layout="RevMan5", sortvar = TE, 
            prediction = FALSE, 
            print.tau2 = FALSE,
            fontsize=7,
            leftlabs = c("study", "SE", "g"), cex=0.78) # sortvar = TE: sort studies by effect size 

### add text for test of overall effect https://www.metafor-project.org/doku.php/plots:forest_plot_revman#code
par(xpd=NA)
#par(cex=sav$cex, font=1)
#text(sav$xlim[1], -4, pos=4, bquote(paste("Test for overall effect: ",
#   "Z = ", .(fmtx(m.gen$zval.random, digits=2)),
#   " (", .(fmtp(m.gen$pval.random, digits=2, pname="P", add0=TRUE, sep=TRUE, equal=TRUE)), ")")))
...
```{r, message=F, warning=F}
netsplit(m.netmeta) %>% forest()  # visualize the net split results 
```



```{r, message=F, warning=F}
funnel(m.netmeta, order=unique(m.netmeta$treat1), pch=seq(1:12), col=seq(1:12), linreg = T, pos.legend = 1)
```
