...
side_by_side_barplot3=function(data, angle=0, num_size=3, x_text_size=10, legend_text_size=10, title, legend_rows=1)
{
  y_limit=max(data$n)*1.1
  g=ggplot(data, aes(x=races, y=n, fill=ages_group)) +geom_bar(position = "dodge", stat="identity")+
  ylim(c(0, y_limit))+
  ylab("n")+xlab("")+
  theme(legend.position="bottom", legend.text = element_text(size=legend_text_size))+
  theme(axis.text.x = element_text(angle = angle, vjust = 0.5, size=x_text_size))+
...
side_by_side_barplot3(age_groups_count_subset, title="age distrbituon by races", num_size=2)
...
chisq.test(genders_group_count[1:2,])
