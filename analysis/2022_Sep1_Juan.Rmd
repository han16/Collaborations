...
result=data_frame(group1_vs_group2=paste(rep(colnames(data)[6:11], each=6), colnames(data)[6:11], sep="-"), mean_difference=round(estimate,2),  CI_lower=round(CI_lower,2),  CI_upper=round(CI_upper,2), p_value=round(p_value,2))  
#knitr::kable(result)
...
```{r, echo=F, message=F, warning=F}
p_value=numeric(); estimate=numeric(); CI_lower=numeric(); CI_upper=numeric(); SD=numeric(); SE=numeric(); minimum=numeric(); maximum=numeric()
for (i in 1:5)
{
