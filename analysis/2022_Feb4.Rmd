...
data_merge=data%>%mutate(diabetes=Diabetes, cancer_treatment=cancer_treatment, thyroid_diseases=thyroid_diseases, Bulimia_acid_reflux_sleep_apnea=Bulimia_acid_reflux_sleep_apnea)
...
########### do not run this one with separate disese subtypes 
...
glm_fit=glm(as.factor(thyroid_diseases)~Age_at_exam  + Sex + county_regroup, data = sub_data, family = "binomial") # response variable need to be factor 
cbind(exp(cbind(OR = coef(glm_fit), confint(glm_fit))), p_value=coef(summary(glm_fit))[,4])
```
* 373/374 have thyroid diseases
...
```{r, echo=F, message=F, warning=F}
sub_data=data_merge%>%filter(cancer_treatment!=2)
glm_fit=glm(as.factor(cancer_treatment)~Age_at_exam  + Sex + county_regroup, data = sub_data, family = "binomial") # response variable need to be factor 
cbind(exp(cbind(OR = coef(glm_fit), confint(glm_fit))), p_value=coef(summary(glm_fit))[,4])
```

* 317/484 receive either radiation or chemotherapy
...
```{r, echo=F, message=F, warning=F}
sub_data=data_merge%>%filter(med_history_tobacco!=2)
glm_fit=glm(as.factor(med_history_tobacco)~permanent_teeth_missing  + teeth_affected_caries , data = sub_data, family = "binomial") # response variable need to be factor 
cbind(exp(cbind(OR = coef(glm_fit), confint(glm_fit))), p_value=coef(summary(glm_fit))[,4])
```
* 2582/5852 patients are tobacco users 
