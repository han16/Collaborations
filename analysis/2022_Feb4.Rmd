...
glm_fit=glm(as.factor(med_history_arthritis)~Age_at_exam  + Sex + county_regroup, data = sub_data, family = "binomial")
cbind(exp(cbind(OR = coef(glm_fit), confint(glm_fit))), p_value=coef(summary(glm_fit))[,4])

```

* use 1160/1657 patients with arthritis by `med_history_arthritis!=2`
...
glm_fit=glm(as.factor(Bulimia_acid_reflux_sleep_apnea)~Age_at_exam  + Sex + county_regroup, data = sub_data, family = "binomial")
cbind(exp(cbind(OR = coef(glm_fit), confint(glm_fit))), p_value=coef(summary(glm_fit))[,4])

```

* 1046/2507 have `Bulimia_acid_reflux_sleep_apnea==1`
...
* 553/563 patients have depression 
...
glm_fit=glm(as.factor(diabetes)~Age_at_exam  + Sex + county_regroup, data = sub_data, family = "binomial") # response variable need to be factor 
cbind(exp(cbind(OR = coef(glm_fit), confint(glm_fit))), p_value=coef(summary(glm_fit))[,4])
