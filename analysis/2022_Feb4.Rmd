...
count_group=numeric()
count_group[1]=data%>%filter(Sex==1)%>%nrow()
count_group[2]=data%>%filter(Sex==2)%>%nrow()

ggplot(data.frame(gender=gender_group, count=count_group), aes(x=gender, y=count, fill=gender)) +geom_bar(stat="identity")+
  theme(legend.position="none")+
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5, size=12))+
...
#knitr::kable(data.frame(gender=gender_group, count=count_group), caption = "Patients distribution by age")%>%
#kable_styling() %>%
#scroll_box(height = "200px")
```
...
ggplot(data.frame(variables=paste(seq(1,9), variables, sep="_"), missing_prop=variables_missing_prop), aes(x=variables, y=variables_missing_prop, fill=variables)) +geom_bar(stat="identity")+
  ...
  geom_text(aes(label=round(variables_missing_prop,4)), position=position_dodge(width=0.9), vjust=-0.25)+  # add numbers over bars 
  theme(plot.title = element_text(hjust = 0.5, size=15))  #center the title 
...
```
1) natural dentition graph and natural dentition with age brackets graph on our patient demographic  (similar to slides 14 and 15 of the UIC presentation we shared with you). 
2) Information on active caries a) by age and by b)county
3) Medical conditions (predictors) by age, and by county
4) smoking/tobacco- effect on number of missing teeth in our patient population
```
...
ggplot(data.frame(teeth=teeth_group, proportion=teeth_count/nrow(data)), aes(x=teeth, y=proportion, fill=teeth)) +geom_bar(stat="identity")+
  theme(legend.position="none")+
  xlab("")+ylab("Patients(%)")+
  ylim(c(0,1))+
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5, size=12))+
