...
```{r, message=F, warning=F, eval=F}
# Fit the Cox model
cox_fit <- coxph(Surv(Time, Event) ~ Tooth_Type+Age+Sex+SSC, data = SDF_data_tooth_type) # SSC has 0 success, causing the computing issue
...
library(coxphf)
firth_fit <- coxphf(Surv(Time, Event) ~ Tooth_Type + Age + Sex + SSC, 
                    data = SDF_data_tooth_type)
...
# Define variables
explanatory = c("Tooth_Type", "Age", "Sex", "SSC")
...
# Create the plot
SDF_data_tooth_type %>%
  hr_plot(dependent, explanatory, table_text_size = 4, title = "Hazard Ratios (Firth Penalized)")
```


* age, gender, SSC and teeth type are included in the model 

👶 Age
HR = 0.854, p = 0.046 
Interpretation: Each 1-unit increase in age (one month) is associated with a 14.6% decrease in hazard of failure

Suggests older children may have better treatment survival

🚻 Sex
HR = 0.52, p = 0.0522 male=1 , female=2

* Female teeth had a 48% lower hazard of failure compared to male teeth, and this difference is not  statistically significant.

🦷 SSC (Stainless Steel Crown)
HR=0.059, p=0.001

Interpretation: This indicates that having an SSC is associated with a 94.1% reduction in the hazard (risk) of "Failure" compared to not having an SSC, after controlling for age, sex, and tooth type.

Tooth Type: None of the specific tooth types show a statistically significant difference compared to the reference group (Anterior)


* Firth’s penalized likelihood Cox regression was used to account for monotone likelihood (zero events in the SSC group)
...
# 1. Extract results from the firth_fit object
# We use exp() to convert coefficients to Hazard Ratios
results_table <- data.frame(
  Covariate = names(firth_fit$coefficients),
  HR = exp(firth_fit$coefficients),
...
final_table <- results_table %>%
  mutate(
    # Round HR and CIs to 2 decimal places for readability
    HR = round(HR, 2),
  ...
  select(Covariate, HR, `95% CI`, P_Value)

coef_1st_molar <- 0.03581522
se_1st_molar <- 0.64005957

z_score <- abs(coef_1st_molar / se_1st_molar)
p_val_wald <- 2 * (1 - pnorm(z_score))


final_table[1,4]=round(p_val_wald,4)
...
* Age is significant predictors of treatment survival, but not sex 

* Tooth type loses significance after adjustment, except a marginal trend for lower 2nd molars.

* SSC suggests a strong significant protective effect.
