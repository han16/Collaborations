...
gender_zones_data=data.frame(geo=rep(zones,each=2), num=as.vector(gender_prop), gender=rep(c("Man", "Woman"), length(zones))) %>% drop_na()


man_total=sum(gender_zones_data %>% filter(gender=="Man") %>% select(num) %>% pull())
woman_total=sum(gender_zones_data %>% filter(gender=="Woman") %>% select(num) %>% pull())
prop=numeric()
prop[seq(1,9,by=2)]=gender_zones_data$num[seq(1,11,by=2)]/man_total
prop[seq(2,10,by=2)]=gender_zones_data$num[seq(2,12,by=2)]/woman_total
gender_zones_data=gender_zones_data %>% mutate(prop=round(prop,4))

fig=side_by_side_barplot(gender_zones_data, angle=0, num_size = 3, x_text_size=12, legend_text_size=12, title="Dentist_Survey", custom_colors= c("Woman" = "red", "Man" = "blue"))
fig
