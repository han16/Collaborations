...
data=read_xlsx((file.path(root, "../2026/202601/Shikha/Results -2026-01-22.xlsx")))
...
* [version 2-2026-02-10](2026_0101_Shikha_01.html)
...
data <- data %>%
  ...
  ) %>% select(`RCT TreatmentDate`, POSTDate, RCT_to_treatment_days, patient_tooth_id, `Fail Date`, `Fail Code`)
...
last_followup_date <- max(
  c(endo_with_failure$failure_date, endo_with_failure$POSTDate),
  na.rm = TRUE
)
...
p <- ggsurvplot(
  ...
  risk.table = TRUE,
  risk.table.height = 0.4,
  risk.table.fontsize = 5,       # shrink numbers
  break.x.by = NULL               # disable automatic yearly breaks

)

# Apply the custom breaks and labels
p$plot <- p$plot +
  scale_x_continuous(
    breaks = day_breaks,
    labels = paste0("Year ", year_points)
